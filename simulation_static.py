        ...
        for j in range(len(vehicles[directionNumbers[nextGreen]][i])):
            ...
            if(vehicle.crossed==0):
                ...
                if(vclass=='car'):
                    ...
                    noOfRickshaws += 1
    # print(noOfCars)
    # STATIC CHANGING HERE
    greenTime = 30
    # greenTime = math.ceil((noOfVehicles)/noOfLanes)
    # PERFORM REINFORCEMENT LEARNING HERE
...
def printStatus():
    ...
    with open('STATUS_STATIC_'+ str(now1.hour) + '.' + str(now1.minute) + "date." + str(now1.day) +'.txt' , 'a') as f:
        f.writelines('Total VEhicle Weight time : ' + str( VEHICLE_WAIT_TIMES)+ '\n')
        f.writelines('Total vehicles passed: '+ str(totalVehicles) + '\n')
        f.writelines('wait time per vehicle - ' + str( VEHICLE_WAIT_TIMES/(totalVehicles+1) ) + '\n')
...
def simulationTime():
    ...
    while(True):
        ...
        if(timeElapsed==simTime):
            totalVehicles = 0
            global now1
            with open('STATIC_'+ str(now1.hour) + '.' + str(now1.minute) + "date." + str(now1.day) +'.txt' , 'w') as f:
                ...
                f.write('No. of vehicles passed per unit time: ' + str((float(totalVehicles)/float(timeElapsed)))+ '\n')
