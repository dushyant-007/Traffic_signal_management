...
carTime = 2/TIME_SCALE
bikeTime = 1/TIME_SCALE
rickshawTime = 2.25/TIME_SCALE
busTime = 2.5/TIME_SCALE
truckTime = 2.5/TIME_SCALE
...
speeds = {'car':2.25*TIME_SCALE, 'bus':1.8*TIME_SCALE, 'truck':1.8*TIME_SCALE, 'rickshaw':2*TIME_SCALE, 'bike':2.5*TIME_SCALE}  # average speeds of vehicles
        ...
        if (self.direction == 'right'):
            ...
            if (self.willTurn == 1):
                ...
                if ((self.y >= self.stop or self.crossed == 1 or (currentGreen == 3 and currentYellow == 0)) and (
                    ...
                    self.y -= self.speed
                    moved = True
        if moved == False: VEHICLE_WAIT_TIMES += 1
...
#       greenTime = defaultMaximum
    ...
    # print(noOfCars)
    random_fac = 0.5
    upper_limit = (11 - 1050//100 )/5
    greenTime = math.ceil(((noOfCars*(carTime+random_fac*np.random.uniform(0, upper_limit))) + (noOfRickshaws*(rickshawTime+random_fac*np.random.uniform(0*upper_limit, upper_limit))) + (noOfBuses*(busTime+random_fac*np.random.uniform(0*upper_limit, upper_limit))) + (noOfTrucks*(truckTime+random_fac*np.random.uniform(0*upper_limit, upper_limit)))+ (noOfBikes*(bikeTime+random_fac*np.random.uniform(0*upper_limit, upper_limit)))))/(noOfLanes + 1)
...
def generateVehicles():
    ...
    while(True):
        ...
        if timeElapsed%100 == 0 :
            a = [random.randint(0,999), random.randint(0,999), random.randint(0,999), random.randint(0,999)]
            for i in range(1,4):
                a[i] = a[i]+a[i-1]
            for i in range(0,4):
                a[i] = a[i]/a[3]*1000
        ...
        time.sleep(random.randint(1,4)/TIME_SCALE/VEHICLE_GEN_MULTIPLIER)
...
def simulationTime():
    ...
    while(True):
        ...
        if (timeElapsed == simTime):
            ...
            with open('REINFORCE_' + str(now1.hour) + '.' + str(now1.minute) + "date." + str(now1.day) + '.txt', 'w') as f:
                f.writelines('Lane-wise Vehicle Counts\n')
                f.writelines('Lane-wise Vehicle Counts\n')
                for i in range(noOfSignals):
                    f.write('Lane' + str(i + 1) + ':' + str(vehicles[directionNumbers[i]]['crossed']) + '\n')
                    totalVehicles += vehicles[directionNumbers[i]]['crossed']
                f.write('Total vehicles passed: ' + str(totalVehicles) + '\n')
                f.write('Total VEhicle Weight time : ' + str(VEHICLE_WAIT_TIMES) + '\n')
                f.write('Total time passed: ' + str(timeElapsed) + '\n')
                f.write(
                    'No. of vehicles passed per unit time: ' + str((float(totalVehicles) / float(timeElapsed))) + '\n')
                f.write('Wait time per Vehicle : ' + str(VEHICLE_WAIT_TIMES/totalVehicles) + '\n')
